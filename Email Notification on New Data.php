...
function email_notification_on_new_data_register_settings_page() {
    add_options_page(
        'Email Notification on New Data',
        'Email Notification on New Data',
        'edit_others_posts',
        'email-notification-on-new-data',
        'email_notification_on_new_data_settings_page'
    );
    add_submenu_page(
        'email-notification-on-new-data',
        ...
        'Select Database Table',
        'edit_others_posts',
        'email-notification-on-new-data-table',
        'email_notification_on_new_data_table_page'
    );
}



add_action('admin_menu', 'email_notification_on_new_data_register_settings_page');
...
<p><a href="<?php echo admin_url('options-general.php?page=select-database-table'); ?>">Select a database table</a></p>
            ...
            <td>
                <select name="email_notification_on_new_data_table">
                    <option value="">Select a table</option> <?php foreach ($tables as $table) : ?> <option
                        value="<?php echo $table; ?>" <?php selected($selected_table, $table); ?>> <?php echo $table; ?>
...
function email_notification_on_new_data_data_page() {
    global $wpdb;
    $table = get_option("email_notification_on_new_data_table");
    
    if (empty($table)) { 
        echo '<div class="notice notice-warning is-dismissible"><p>Please select a database table in the plugin settings.</p></div>';
        return;
    }
...
function email_notification_on_new_data_update_data() {
    global $wpdb;
    $table = get_option('email_notification_on_new_data_table');
    $email = get_option('email_notification_on_new_data_email');
    ...
    if (empty($table) || empty($email) || empty($email_body_template)) {
        wp_die('Error: please ensure all required settings are configured.');
    }
    $data = $_POST['email_notification_on_new_data_data'];
    if (empty($data)) {
        wp_die('Error: please select at least one row of data.');
    }
    $email_body = '';
    $results = $wpdb->get_results("SELECT * FROM $table WHERE id IN (" . implode(',', array_map('intval', $data)) . ")", ARRAY_A);
    $columns = array_keys($results[0]);
    foreach ($results as $result) {
        $email_body .= $email_body_template . "\n\n";
        foreach ($columns as $column) {
            $email_body = str_replace("{{$column}}", $result[$column], $email_body);
        }
    }
    wp_mail($email, "Selected data from {$table}", $email_body);
    wp_redirect(admin_url('options-general.php?page=email-notification-on-new-data'));
    exit;
}
// Hook into the “shutdown” action to monitor the table for new data
add_action('shutdown', 'email_notification_on_new_data_on_insert', 0);
...
$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$table}"), ARRAY_A);
...
function email_notification_on_new_data_table_page() {
    global $wpdb;
    $tables = $wpdb->get_col("SHOW TABLES");

    if (isset($_POST['email_notification_on_new_data_nonce']) && wp_verify_nonce($_POST['email_notification_on_new_data_nonce'], 'email_notification_on_new_data_select_table')) {
        update_option('email_notification_on_new_data_table', sanitize_text_field($_POST['email_notification_on_new_data_table']));
        echo '<div class="notice notice-success is-dismissible"><p>Table selected.</p></div>';
    }
    $selected_table = get_option('email_notification_on_new_data_table');
    ?> <div class="wrap">
    <h1>Select Database Table</h1>
    <form method="post" action="">
        <?php wp_nonce_field('email_notification_on_new_data_select_table', 'email_notification_on_new_data_nonce'); ?>
        <table class="form-table">
            <tr valign="top">
                <th scope="row">Database Table:</th>
                <td>
        ...
        <p class="submit"> <input type="submit" class="button-primary" value="Select Table" /> </p>
    </form>
</div> <?php
}
